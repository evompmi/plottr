...
| `tools/stats.js`            | 3329    | 2096 + 77 timed out | 1054 + 102 no-coverage                        | **65.27% raw / 67.34% covered** | Largest scope so far. Three passes: file-skip → selective skip → property tests for multiset / clustering + power-loader refactor + `Stryker disable` annotations on equivalent-mutant validators. No-coverage dropped 528 → 102 (-81%). See `docs/testing-2026-05-08.md`. |
...
- **New shared function** in `shared.js` / any `stats-*.js` / any `shared-*.js` → export from the matching loader in `tests/helpers/` and add unit tests to the appropriate `tests/*.test.js` file (or create a new one if the domain is new). Stats source loading goes through `tests/helpers/stats-source.js`'s `STATS_FILES` list; if you add a new `stats-*.js` file, append it there in load order.
