...
- **The React surface is `any`-free end to end** — no `: any` annotations or destructures, no `any` inside `useState` / `useRef` generics or `Record` / `Map` value types, no `as any` escape-hatch casts — and the repo carries no `@ts-ignore` / `@ts-expect-error` / `@ts-nocheck` directives anywhere. Keep it that way: if a prop bag or a piece of component state feels unwieldy, lift the shape into a typed `interface` instead of reaching for `any`. Prop interfaces live in the tool's `helpers.ts`; component-internal state shapes (e.g. heatmap's `HoverCell` / `BrushState` / `AxisHover`) can stay local at the top of the `.tsx`. `useRef` for a DOM/SVG node is typed to that element (`useRef<SVGSVGElement>(null)` / `useRef<HTMLDivElement>(null)`), never `useRef<any>`. The remaining `as <Type>` casts in the React tier are deliberate narrowing only — literal-union casts after a runtime check, reducer-patch `Partial<…>`, `ReturnType<…>` — not escape hatches. There is **no** `as any`, but a small, bounded set of `as unknown as` narrowings is allowed and should stay confined to these documented sites: generic `T` ↔ `PrefsRecord` round-tripping in `_shell/prefs-store.ts`, open-registry `run()` return narrowing to `PostHocResult` in `_shell/stats-dispatch.ts` / `_shell/StatsTile.tsx`, and the `window.__plottrSpaNavigate` global registration in `_app/index.tsx`. Don't add new `as unknown as` casts elsewhere — lift a typed shape instead.
...
npm test               # run every tests/*.test.js (full deterministic Vitest suite)
