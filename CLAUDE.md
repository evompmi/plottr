...
- The shared kernel lives as ES modules under `tools/_core/` (numerical / stats / parsing / theme). Each module emits real `export` declarations and adds a trailing `globalThis.X = X` shim so legacy callers (tool `.tsx` files using `parseRaw`, `tTest`, `PALETTE`, …) keep working without an import sweep. The shims are transitional and will retire once every caller imports its symbols directly.
...
npm test               # run every tests/*.test.js (37 deterministic suites, 1,557 tests)
