...
- `index.html` — landing page **and** SPA entry. The static landing markup (tile grid, "How it works" pills, footer with internal-tests count) is HTML; clicking a tile sets `location.hash = "#/<tool>"` and the SPA shell renders the matching tool component into `<div id="root">`. A short inline IIFE near the bottom toggles a `data-spa-route="active"` attribute on `<html>` whenever a route is present, and CSS hides the landing accordingly.
