      ...
      >
        ...
        <div>
          <div style={{ color: "#fff", fontWeight: 700, fontSize: 15 }}>
            Bar Graph Viewer — How to use
          </div>
          <div style={{ color: "rgba(255,255,255,0.75)", fontSize: 11, marginTop: 2 }}>
            Long or wide data → auto-detect → mean ± SEM/SD bar charts
          </div>
        </div>
      </div>
      <div
      ...
      >
        ...
        >
          ...
          <p style={{ fontSize: 12, lineHeight: 1.75, color: "#444", margin: 0 }}>
            Bar chart visualization with mean ± SEM/SD error bars and optional jittered data points.
            Accepts <strong>both long and wide formats</strong>. Wide data goes straight to plot;
            long data gets the full configure → filter → output → plot pipeline.
          </p>
        </div>
        <div
          style={{
        ...
        >
          ...
          <p style={{ fontSize: 11, color: "#555", marginBottom: 8, lineHeight: 1.6 }}>
            Each <strong>row</strong> = one observation. Mix of categorical and numeric columns.
          </p>
        ...
        >
          ...
          <p style={{ fontSize: 11, color: "#555", marginBottom: 8, lineHeight: 1.6 }}>
            One <strong>column</strong> per condition. All numeric.{" "}
        ...
        >
          ...
          <span style={{ fontSize: 11, color: "#444" }}>
            Duplicate column names in wide format are pooled as replicates. Points are colored by
            source column shade.
          </span>
        </div>
...
function PlotControls({
  ...
  const handleDownloadSvg = () => {
    if (facetByCol >= 0 && dataFormat === "long" && facetedData.length > 0) {
      facetedData.forEach((fd) =>
        downloadSvg(facetRefs.current[fd.category], `bargraph_${fd.category}.svg`)
      );
    } else {
  ...
  const handleDownloadPng = () => {
    if (facetByCol >= 0 && dataFormat === "long" && facetedData.length > 0) {
      facetedData.forEach((fd) =>
        downloadPng(facetRefs.current[fd.category], `bargraph_${fd.category}.png`)
      );
    } else {
      ...
      {dataFormat === "wide" && (
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ margin: "2px 0 0", fontSize: 10, color: "#047857" }}>
              Duplicate headers pooled as replicates.
            </p>
          </div>
        </div>
      )}
        ...
        {effectiveGroups.some((g) => g.sources.length > 1) && (
          ...
          >
            ...
            <div>
              ...
              <p style={{ margin: "2px 0 0", fontSize: 10, color: "#b45309" }}>
                Values from duplicate columns have been pooled as replicates. Jitter points are
                shaded by source column.
              </p>
            </div>
          </div>
        )}
...
function App() {
  ...
  const facetedData = useMemo(() => {
    ...
    return facetByCategories.map((cat) => {
      ...
      const groups = effectiveOrder
        .filter((n) => rawMap[n] && !disabledGroups[n])
        .map((n) => rawMap[n]);
  ...
  const allSteps =
    dataFormat === "long"
      ? ["upload", "configure", "filter", "output", "plot"]
      : ["upload", "plot"];
      ...
      {step === "plot" && canPlot && (
        <div style={{ display: "flex", gap: 20, alignItems: "flex-start" }}>
          <PlotControls
            ...
            renamedRows={renamedRows}
            colNames={colNames}
            facetByCandidates={facetByCandidates}
