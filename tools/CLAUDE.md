...
The repository ships **nine plot tools** (each in its own folder) and **two single-file calculators**, all served from a single hash-routed SPA: one `index.html`, one ReactDOM mount, one bundle.
...
| Genome Track              | `tools/gff/`      | GFF3 / GFF feature track — parses the 9-column format + ID/Parent hierarchy, packs features into lanes. |
