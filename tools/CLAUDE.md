...
The repository ships **eight plot tools**, **one stats-only tool** (Factorial Analysis), and **two single-file calculators**, all served from a single hash-routed SPA: one `index.html`, one ReactDOM mount, one bundle.
...
| Factorial Analysis        | `tools/factorial/` | Two-factor (A × B) ANOVA with Type II sums of squares — stats-only, no chart.                          |
...
artefact is now just the bundled `_core/theme.ts` (so the no-FOUC inline
IIFE in each `<head>` has the theme controls available before the SPA
module bundle parses). The bundle is regenerated by
`scripts/build-shared.js` in `prebuild` / `prewatch` / `pretest` and is
checked into git for the static GitHub Pages deploy.
...
- **`tools/_core/theme.ts`** — theme toggle wiring + `ThemeToggle` React component (`getTheme`, `setTheme`, `toggleTheme`, `useThemeMode`). Bundled separately by `scripts/build-shared.js` because the inline no-FOUC IIFE in each HTML `<head>` needs theme state available before the SPA module bundle parses.
...
- **`tools/factorial/`** has no `chart.tsx` and no `plot-area.tsx`. It's a stats-only tool (Type II two-way ANOVA): the "plot" step is named "report" and renders the formatted ANOVA table (`report.tsx`) instead of an SVG. `PlotToolShell`'s new `showPrefsOnStep` prop lets the PrefsPanel attach to "report" instead of the hardcoded "plot". The `EXAMPLE_CSV` convention still applies; the role union is local (`factorA / factorB / value / ignore`, not the shared `group / value / filter / ignore`) so the configure step builds its own local role editor in `steps.tsx`.

**Sample-data convention — "all-(C)" / inline at module scope.** Every plot tool's `app.tsx` exposes a "Try sample data" button. The dataset that powers it lives as a ``const EXAMPLE_CSV = `…`;`` (or `EXAMPLE_TSV` for tab-separated, or `(() => { … })()` if procedurally generated) at the **top of `app.tsx`**, immediately after the imports and before `App()`. The button's handler is a `useCallback` named `loadExample` that calls `setSepOverride(",")`, `setFileName("…")`, and `doParse(EXAMPLE_CSV, ",")`. This convention is non-negotiable and applies everywhere — `aequorin`, `boxplot`, `factorial`, `heatmap`, `lineplot`, `scatter`, `upset`, `venn`, `volcano`. Why it matters:
...
All eight plot tools (Aequorin, Boxplot, Lineplot, Scatter, Heatmap, Venn, UpSet, Volcano) plus the stats-only Factorial Analysis tool use the shared scaffold under `tools/_shell/`. Unlike the plain-JS `shared-*.js` globals, these are TypeScript modules imported via `import { … } from "./_shell/…"` and resolved by esbuild when bundling each tool. The two calculators (`molarity-app.tsx`, `power-app.tsx`) intentionally do **not** use this scaffold — they have no upload step, no column roles, no step navigator, so the shell would be dead weight. Factorial uses the same scaffold with `showPrefsOnStep="report"` so the PrefsPanel attaches to its analysis step (named "report", not "plot").
