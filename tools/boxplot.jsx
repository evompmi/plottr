...
const BoxplotChart=forwardRef(function BoxplotChart({groups,yLabel,plotTitle,plotBg,showGrid,gridColor,boxWidth,boxFillOpacity,pointSize,showPoints,jitterWidth,pointOpacity,xLabelAngle,yMin:yMinP,yMax:yMaxP,categoryColors:catCols,colorByCol:cbc,boxGap,svgLegend,showCompPie},ref){
  ...
  return(
    <svg ref={ref} viewBox={`0 0 ${vbW} ${vbH}`} style={{width:vbW,maxWidth:"100%",height:"auto",display:"block"}} xmlns="http://www.w3.org/2000/svg">
      ...
      {groups.map((g,gi)=>{const lx=bx(gi),ly=M.top+h+16;const compBar=(()=>{if(cbc<0||!g.sources||!showCompPie)return null;const total=g.allValues.length;if(!total)return null;const r=20,cy2=vbH_chart-r-12;let cum=0;const slices=g.sources.map((src,si)=>{const pct=src.values.length/total;const a0=cum*Math.PI*2,a1=(cum+pct)*Math.PI*2;cum+=pct;const col=(catCols&&src.category)?catCols[src.category]||"#999":"#999";if(pct>=1)return<circle key={si} cx={lx} cy={cy2} r={r} fill={col}/>;const x0=lx+Math.sin(a0)*r,y0=cy2-Math.cos(a0)*r,x1=lx+Math.sin(a1)*r,y1=cy2-Math.cos(a1)*r;const lg=pct>0.5?1:0;return<path key={si} d={`M${lx},${cy2}L${x0},${y0}A${r},${r},0,${lg},1,${x1},${y1}Z`} fill={col}/>;});const labels=g.sources.map((src,si)=>{const pct=src.values.length/total;if(pct<0.08)return null;const cumPct=g.sources.slice(0,si).reduce((s,ss)=>s+ss.values.length/total,0);const midA=(cumPct+pct/2)*Math.PI*2;const lr=r+8;return<text key={`t${si}`} x={lx+Math.sin(midA)*lr} y={cy2-Math.cos(midA)*lr+3} textAnchor="middle" fontSize="7" fill="#888" fontFamily="sans-serif">{Math.round(pct*100)}%</text>;});return<g key={`cb-${g.name}`}>{slices}<circle cx={lx} cy={cy2} r={r} fill="none" stroke="#ddd" strokeWidth="0.5"/>{labels}</g>;})();return<React.Fragment key={`xl-${g.name}`}>{angle===0?(<g><text x={lx} y={ly} textAnchor="middle" fontSize="11" fill="#333" fontFamily="sans-serif" fontWeight="600">{g.name}</text><text x={lx} y={ly+14} textAnchor="middle" fontSize="9" fill="#999" fontFamily="sans-serif">n={g.stats?.n||0}</text></g>):(<g transform={`rotate(${angle},${lx},${ly})`}><text x={lx} y={ly} textAnchor="end" dominantBaseline="middle" fontSize="11" fill="#333" fontFamily="sans-serif" fontWeight="600">{g.name}</text><text x={lx} y={ly+12} textAnchor="end" dominantBaseline="middle" fontSize="9" fill="#999" fontFamily="sans-serif">n={g.stats?.n||0}</text></g>)}{compBar}</React.Fragment>;})}
