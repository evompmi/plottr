...
- **Stats-summary SVG print now opt-in** — the text summary that was baked into exported SVGs whenever "Display on plot" was on could eat plot space on large multigroup comparisons (especially post-hoc tables with many pairs). The Statistics display tile now has a separate "Print summary below plot" checkbox, unchecked by default, so turning on-plot annotations on no longer implicitly lengthens the canvas. The display-tile controls ("Display on plot", "Print summary below plot", Style radios, "Show ns") are also now all visible simultaneously rather than revealing conditionally, so users can see every available toggle at a glance. Applies to Group Plot and Aequorin (both load `shared-stats-tile.js`).
...
- **Aequorin barplot wrapper tile stayed bright (dimmed) white in dark mode** — the `IntegralTile` that hosts the inset barplot section was marked as `.dv-plot-card` and painted itself with `var(--plot-card-bg)` / `var(--plot-card-border)`, which are hard-coded white + light-grey in both themes and only get softened in dark mode via `filter: brightness(0.85)`. That design makes sense for tiles that _are_ the plot canvas, but `IntegralTile` also carries a lot of chrome around the SVG — the "Barplot (Σ of plotted values)" disclosure header, the Σ Raw / Σ Baseline-corrected toggle, the per-replicate CSV table, and the full `StatsTile` — all of which were forced to sit on a dimmed-white surface against the dark UI instead of blending with it. Swapped the outer tile to themed `var(--surface)` + `var(--border-strong)` (both flip to dark values via the `[data-theme="dark"]` overrides in `theme.css`) and dropped its `.dv-plot-card` class so it no longer gets the brightness filter. The inner wrapper around `<InsetBarplot/>` is now the `.dv-plot-card` instead, keeping the bar plot canvas itself white-and-dimmed for export consistency while the surrounding chrome correctly follows dark mode.
