...
- **`tinv` now correctly resolves heavy-tail / extreme quantiles instead of clamping to ±50** — the bisection bounds were hard-coded `[-50, 50]`, which silently clamped any quantile whose true value exceeded that range. The worst case was the Cauchy distribution (df=1): `tinv(0.001, 1)` was returning `-50` instead of `-318.3`. Lower but still plausible cases like `tinv(1e-6, 3)` also clamped. This could only corrupt p-value → critical-value inversions at extreme significance levels or very low df — the tool's default Welch/Tukey flows use higher df and central quantiles, so the bug was unlikely to have misled a scientific result in practice, but the fix removes an unbounded silent failure mode. `tinv` now starts from `[-1, 1]` and doubles each bound outward until it brackets the target — same approach already used by `chi2inv`. Verified against R's `qt()` to 7+ significant digits across df ∈ {1, 2, 3, 10, 30} and p ∈ {1e-10 … 1 − 1e-10}. Covered by four new tests in `tests/stats.test.js`.
