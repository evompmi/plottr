...
- **EC50 / IC50 dose–response tool — new plot.** 4-parameter logistic
  ...
  EC50 / shared Hill across conditions. Three-step flow:
  Upload → Configure (data preview + role assignment + fit options) →
  Plot (curves with 95% CI ribbon, residuals diagnostic, in-SVG
  parameter table). Long and wide CSV shapes both supported; explicit
  warnings on poor identifiability and non-convergence — no silent
  fallbacks. (regression: 55 tests)
