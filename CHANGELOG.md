...
- **Heatmap — cell tooltips, interactive selection, and a linked detail panel** — hovering any cell in the main heatmap now shows a floating tooltip with the row label, column label, raw value, and (when normalisation is active) the plotted/normalised value. Three ways to build a selection: (1) drag a rectangular brush on the main grid; (2) click any internal branch of a row or column dendrogram to select that subtree's leaves; (3) in k-means mode, click a segment on the cluster-assignment strip to select every row/column sharing that cluster id. Selections highlight as translucent row/column bands on the main plot and drive a second `HeatmapChart` rendered below the main one, showing only the selected rows × columns with its own SVG / PNG / CSV download buttons and a data table of the raw values (capped at 50 visible rows; CSV always has the full selection). Selection indices point into the original unordered matrix, so re-clustering doesn't silently mis-highlight — any change to mode / k / distance / linkage / normalisation auto-clears the selection. The detail chart itself isn't interactive (tooltips only) to keep selection state single-sourced in the main view. Exporting the detail SVG gives a touch-up-ready figure of just the subset without having to go back to the source data and filter it by hand.
